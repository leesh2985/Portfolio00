...
export default function ToolsPosts({ loading, startIndex, endIndex }: PostProps) {
  const [posts, setPosts] = useState<{ id: number; title: string; userId: string }[]>([]);

  useEffect(() => {
    const fetchData = async () => {
      const querySnapshot = await getDocs(query(collection(dbService, 'Tools'), orderBy('id', 'desc')));
